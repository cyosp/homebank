...
GVariant *variant = g_action_group_get_action_state(G_ACTION_GROUP(data->action_group_top), "viewby");

	DB( g_print(" new type is %s\n", g_variant_get_string (variant, NULL) ) );

	issubcat = FALSE;
	if( !strcmp("subcat", g_variant_get_string(variant, NULL)) )
		issubcat = TRUE;
	...
	GQueue *txn_queue;
			...
			gdouble trn_amount;
				...
				struct tmptop *item;
					...
					for(i=0;i<nbsplit;i++)
					{
						...
						pos = category_report_id(split->kcat, issubcat);
						if( pos <= garray->len )
						{
				...
				struct tmptop *item;
					...
					pos = category_report_id(ope->kcat, issubcat);
					if( pos <= garray->len )
					{
						//#1297054 if( trn_amount < 0 ) {
							item = &g_array_index (garray, struct tmptop, pos);
		...
		n_items = MIN(garray->len, (guint)PREFS->rep_maxspenditems);
		other = 0;
		...
		struct tmptop *item;
			...
			if(item->value < 0)
			{
				...
				DB( g_print(" - %d : k='%d' v='%f' t='%f' o='%f'\n", i, item->key, item->value, total, other) );
...
static void
ui_hub_spending_activate_radio (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
...
//	name, function(), type, state, 
	{ "viewby", ui_hub_spending_activate_radio ,  "s", "'cat'", NULL, {0,0,0} }
};
...
GtkToolItem *toolitem;
	...
	//widget = hbtk_radio_button_new(GTK_ORIENTATION_HORIZONTAL, CYA_CATSUBCAT, TRUE);
	//data->RA_type = widget;
...
GMenu *menu, *section;
	...
	menu = g_menu_new ();
	section = g_menu_new ();
	g_menu_append (section, _("Category")       , "actions.viewby::cat");
	g_menu_append (section, _("Subcategory")    , "actions.viewby::subcat");
	g_menu_append_section(menu, NULL, G_MENU_MODEL(section));
	g_object_unref (section);


	GSimpleActionGroup *group = g_simple_action_group_new ();
	data->action_group_top = group;
	...
	//hbtk_radio_button_connect (GTK_CONTAINER(data->RA_type), "toggled", G_CALLBACK (ui_hub_spending_populate), NULL);
