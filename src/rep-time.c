...
static void reptime_cb_acc_changed(GtkCellRendererToggle *cell, gchar *path_str, gpointer user_data);
static void reptime_cb_cat_changed(GtkCellRendererToggle *cell, gchar *path_str, gpointer user_data);
...
static void reptime_action_viewlist(GtkToolButton *toolbutton, gpointer user_data)
{
...
static void reptime_action_viewcolumn(GtkToolButton *toolbutton, gpointer user_data)
{
struct reptime_data *data = user_data;
...
//beta
#if MYDEBUG
static void reptime_action_filter(GtkToolButton *toolbutton, gpointer user_data)
{
struct reptime_data *data = user_data;
	...
	if(ui_flt_manage_dialog_new(GTK_WINDOW(data->window), data->filter, TRUE, FALSE) != GTK_RESPONSE_REJECT)
	{
		...
		hbtk_combo_box_set_active_id(GTK_COMBO_BOX_TEXT(data->CY_range), FLT_RANGE_MISC_ALLDATE);
...
static void reptime_export_result_clipboard(GtkWidget *widget, gpointer user_data)
{
struct reptime_data *data;
...
static void reptime_export_result_csv(GtkWidget *widget, gpointer user_data)
{
struct reptime_data *data;
gchar *filename = NULL;
GString *node;
...
static void reptime_export_detail_clipboard(GtkWidget *widget, gpointer user_data)
{
struct reptime_data *data;
GtkClipboard *clipboard;
GString *node;
...
Filter *flt;
	...
	switch(tmpsrc)
	{
		...
		case REPORT_SRC_TAG:
			...
			//da_flt_status_tag_set(flt, 0, TRUE);
			break;
		default: //REPORT_SRC_ACCOUNT
			pageid = 0;
...
static void reptime_zoomx_callback(GtkWidget *widget, gpointer user_data)
{
struct reptime_data *data;
gdouble value;
...
GtkWidget *toolbar, *button;
	...
	g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (reptime_export_result_clipboard), data);

	menuitem = gtk_menu_item_new_with_mnemonic (_("_Result to CSV"));
	gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
	g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (reptime_export_result_csv), data);

	menuitem = gtk_menu_item_new_with_mnemonic (_("_Detail to clipboard"));
	data->MI_detailtoclip = menuitem;
	gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
	g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (reptime_export_detail_clipboard), data);

	menuitem = gtk_menu_item_new_with_mnemonic (_("_Detail to CSV"));
	data->MI_detailtocsv = menuitem;
	gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
	g_signal_connect (G_OBJECT (menuitem), "activate", G_CALLBACK (reptime_export_detail_csv), data);
...
static void reptime_window_setup(struct reptime_data *data, guint32 accnum)
{
	DB( g_print("\n[reptime] window setup\n") );
    ...
    data->handler_id[HID_REPTIME_MINDATE] = g_signal_connect (data->PO_mindate, "changed", G_CALLBACK (reptime_date_change), (gpointer)data);
    data->handler_id[HID_REPTIME_MAXDATE] = g_signal_connect (data->PO_maxdate, "changed", G_CALLBACK (reptime_date_change), (gpointer)data);
	data->handler_id[HID_REPTIME_RANGE] = g_signal_connect (data->CY_range, "changed", G_CALLBACK (reptime_range_change), NULL);
	g_signal_connect (data->CY_src, "changed", G_CALLBACK (reptime_for), (gpointer)data);
	data->handler_id[HID_REPTIME_VIEW] = g_signal_connect (data->CY_intvl, "changed", G_CALLBACK (reptime_compute), (gpointer)data);
	g_signal_connect (data->CM_showempty, "toggled", G_CALLBACK (reptime_compute), NULL);

	g_signal_connect (data->RG_zoomx, "value-changed", G_CALLBACK (reptime_zoomx_callback), NULL);
...
#if MYDEBUG
	g_signal_connect (G_OBJECT (data->BT_filter), "clicked", G_CALLBACK (reptime_action_filter), (gpointer)data);
#endif
...
gint row;
	...
	row++;
	label = make_label_group(_("Item filter"));
	gtk_grid_attach (GTK_GRID (table), label, 0, row, 3, 1);
	...
	reptime_sensitive(data->window, NULL);
	reptime_update_detail(data->window, NULL);
